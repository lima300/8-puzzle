...
class Board:
    ...
    def __init__(self, matrix, goal):
        ...
        # validando a entrada goal
        temp = []
        for line in goal:
            temp.extend(line)
        temp.sort()
        if len(temp) != 9 or temp != list(range(size**2)):
            raise ValueError("Invalid goal on constructor")
    ...
    def solve(self):
        ...
        while len(frontier) > 0:
            ...
            if node.board.h() == 0:
                # monta  a lsita de todos os movimentos para a solução recursivamente
                moves = []
                while node.parent:
                ...
                return moves
            # não chegou no estado final ainda
            else:
                # expande o nó  e adciona os novos nós  na fronteira se já estiverem na fronteira
                # ou se ainda não forem explorados
    ...
    def deepening_search(self, time_start, time_limit):
        frontier = [Node(self, None, 0, None)]
        explored = []
        ...
        while True:
            ...
            if node.board._tiles == self._goal:
                moves = []
                while node.parent:
                    moves.append(node.action)
                    node = node.parent
                ...
                return moves
            elif time.time()-time_start > time_limit/2:
                return None
            else:
                # expande o nó  e adciona os novos nós  na fronteira se já estiverem na fronteira
                # ou se ainda não forem explorados
